...
document.addEventListener('DOMContentLoaded', () => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    if (navLoginBtn) {
                        ...
                        navLoginBtn.innerHTML = '<i class="fas fa-user-check"></i><span>Account</span>';
                    }
                }, 1200);
            }, 1500);
        });
    ...
    if (packageSearch && packagesGrid) {
        packageSearch.addEventListener('input', () => {
            ...
            let noResultsMsg = packagesGrid.querySelector('.no-results-message');
            ...
            // Show results info
            if (visibleCount > 0) {
                searchResultsInfo.textContent = `Found ${visibleCount} package${visibleCount > 1 ? 's' : ''} matching "${packageSearch.value}"`;
                if (noResultsMsg) noResultsMsg.remove();
            } else {
                searchResultsInfo.textContent = '';
                if (!noResultsMsg) {
                    ...
                    noResultsMsg.innerHTML = `
                        ...
                        <p>Try searching for "honeymoon", "budget", "family", or "luxury"</p>
                    `;
        ...
        searchClear.addEventListener('click', () => {
            packageSearch.value = '';
            packageSearch.dispatchEvent(new Event('input'));
            packageSearch.focus();
        });
    }
